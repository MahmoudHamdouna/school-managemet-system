...
class AccountSalaryController extends Controller
{
    ...
    public function AccountSalaryGetEmployee(Request $request){
        ...
        foreach ($data as $key => $attend) {
            $account_salary =AccountEmployeeSalary::where('employee_id', $attend->employee_id)->where('date', $attend->date)->first();
            ...
            $html[$key]['tdsource'] .= '<td>' . '<input type="hidden" name="employee_id[]" value="' .
            $attend->employee_id . '">' . '<input type="checkbox" name="checkmanage[]" id="'.$key.'" 
